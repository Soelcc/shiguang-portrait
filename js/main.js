...
function renderGallery(filter,count){
  ...
  grid.innerHTML=visible.map(function(item){
    ...
    if(item.vip&&!isVip){
      return '<div class="work-item locked" onclick="SG_openUnlock()"><div class="work-bg" style="background-image:url('+item.src+')"></div><div class="work-overlay"><span class="lock-badge">🔒 VIP</span><h3>'+item.title+'</h3><span>'+catName+'</span></div></div>';
    }
    return '<div class="work-item"><div class="work-bg" style="background-image:url('+item.src+')"></div><div class="work-overlay"><h3>'+item.title+'</h3><span>'+catName+'</span></div></div>';
  }).join("");
